...
import { ArrowLeft, Play, Pause, Calendar, Sparkles, ChevronRight, ExternalLink } from 'lucide-react';
...
const DAILY_VIDEOS: VideoData[] = [
  { page: 2,  title: '划呀划小船',           enTitle: 'Row Row Row Your Boat',              desc: '坐上小船 gently down the stream，感受水波荡漾的快乐',  category: '自然' },
  { page: 4,  title: '早安大公鸡',             enTitle: 'Good Morning, Mr. Rooster',          desc: '跟着大公鸡学习早上好怎么说，开启美好的一天',          category: '日常' },
  { page: 7,  title: '头肩膝脚趾',             enTitle: 'Head Shoulders Knees & Toes',        desc: '边唱边指，认识身体的各个部位',                       category: '身体' },
  { page: 10, title: '老麦克唐纳',             enTitle: 'Old MacDonald Had A Farm',           desc: '参观农场，认识牛、猪、鸭等动物的叫声',               category: '动物' },
  { page: 13, title: '如果你感到幸福',          enTitle: "If You're Happy And You Know It",    desc: '拍拍手、跺跺脚，用动作表达快乐的心情',               category: '情绪' },
  { page: 20, title: '公交车的轮子',            enTitle: 'The Wheels On The Bus',              desc: '轮子在转呀转，雨刮器在刷呀刷，认识公交车部件',       category: '交通' },
  { page: 44, title: '雨呀快走吧',             enTitle: 'Rain Rain Go Away',                  desc: '小雨小雨快走开，好让我出去玩，学习天气相关词汇',     category: '天气' },
];
...
/* 合并所有视频用于查找 */
const ALL_VIDEOS: VideoData[] = [...DAILY_VIDEOS, ...MORE_VIDEOS];
...
function findVideoByPage(page: number): VideoData {
  return ALL_VIDEOS.find((v) => v.page === page) || DAILY_VIDEOS[0];
}
...
export default function DailyVideo() {
  ...
  const defaultPage = DAILY_VIDEOS[todayIndex].page;

  const [currentPage, setCurrentPage] = useState(defaultPage);
  const [isPlaying, setIsPlaying] = useState(false);

  const current = findVideoByPage(currentPage);
  const embedUrl = buildEmbedUrl(currentPage, isPlaying);
  ...
  const handleSelectDay = (dayIdx: number) => {
    setIsPlaying(false);
    setCurrentPage(DAILY_VIDEOS[dayIdx].page);
  };

  const handleSelectVideo = (video: VideoData) => {
    setIsPlaying(false);
    setCurrentPage(video.page);
  };
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-b from-space-navy via-[#0a0a2e] to-space-navy text-white overflow-x-hidden relative">
      ...
      <div className="max-w-lg mx-auto px-4 pt-16 pb-8 relative z-10">
        ...
        >
          <button
            onClick={() => handleSelectDay((todayIndex + 6) % 7)}
          ...
          <div className="flex items-center gap-2 bg-white/5 px-4 py-2 rounded-full border border-white/10">
            ...
            <span className="text-xs text-lavender-mist/40 bg-white/10 px-2 py-0.5 rounded-full font-noto-sc">
              {current.category}
            </span>
          </div>
          <button
            onClick={() => handleSelectDay((todayIndex + 1) % 7)}
        ...
        >
          <div className="flex items-center justify-between mb-2">
            ...
            <a
              href={`https://www.bilibili.com/video/${BILI_BVID}?p=${currentPage}`}
              target="_blank"
        ...
        {/* Weekly schedule - all 7 days clickable */}
        <motion.div
          className="mt-4"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        >
          <p className="font-noto-sc text-sm text-lavender-mist/50 mb-3 text-center">本周动画日程（点击切换）</p>
          <div className="grid grid-cols-7 gap-1">
            {DAILY_VIDEOS.map((v, i) => {
              const isToday = i === todayIndex;
              const isSelected = v.page === currentPage;
              return (
                <button
                  key={i}
                  ...
                  className={`flex flex-col items-center py-2 rounded-xl transition-all ${
                    isToday
                      ...
                      : isSelected
                      ? 'bg-white/15 border border-star-gold/40'
                ...
                >
                  ...
                  <span className={`text-[10px] mt-1 font-noto-sc truncate max-w-full px-1 ${
                    isSelected ? 'text-star-gold' : 'text-lavender-mist/30'
                  }`}>
                    {v.title}
                  </span>
                </button>
        ...
        >
          <p className="font-noto-sc text-sm text-lavender-mist/50 mb-3 text-center">更多经典儿歌（点击直接播放）</p>
          <div className="space-y-2 max-h-64 overflow-y-auto pr-1" style={{ scrollbarWidth: 'thin' }}>
            {MORE_VIDEOS.map((video, i) => {
              const isActive = video.page === currentPage;
              return (
                <motion.button
                  key={i}
                ...
                >
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${
                    isActive ? 'bg-star-gold/30' : 'bg-nebula-purple/30'
                  }`}>
                    {isActive ? (
                      <Pause size={16} className="text-star-gold" />
                    ) : (
        ...
        >
          视频来自 Bilibili Super Simple Songs 合集
        </motion.p>
      </div>
    </div>
  );
